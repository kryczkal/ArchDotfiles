...
local function deep_merge(original_keys, new_keys)
	for key, new_val in pairs(new_keys) do
		...
		if
			original_val
			and type(original_val) == "table"
			and type(new_val) == "table"
			and not vim.tbl_islist(original_val)
			and not vim.tbl_islist(new_val)
		then
			...
			if original_val.name and new_val.name and original_val.name ~= new_val.name then
				error(
					"Keymap conflict: conflicting names for key '"
						.. key
						.. "'. Original: '"
						.. original_val.name
						.. "', New: '"
