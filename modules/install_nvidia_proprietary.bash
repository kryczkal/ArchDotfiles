...
read -rp "Is /boot/grub/grub.cfg the bootloader configuration file? (yes/no): " grub_choice
grub_choice=$(echo "$grub_choice" | tr '[:upper:]' '[:lower:]')
if [[ "$grub_choice" =~ ^(yes|y)$ ]]; then
	read -rp "Is GRUB the bootloader? (yes/no): " bootloader_choice
	...
	if [[ "$bootloader_choice" =~ ^(yes|y)$ ]]; then
		...
		read -rp "Enter the bootloader configuration file path: " bootloader_config_file
	fi
else
	bootloader_config_file=""
	read -rp "Add nvidia-drm.modeset=1 manually and press enter to continue"
fi
